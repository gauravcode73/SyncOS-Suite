...
export default function CloudStorage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl border border-border bg-card">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold px-4 py-2 rounded-xl border border-slate-700 transition-all cursor-pointer"
          >
      ...
      {/* BREADCRUMBS PATHWAY */}
      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold px-4 py-3 bg-slate-900/10 border border-border rounded-xl">
        <button
          onClick={() => setCurrentFolderId(null)}
          className={`hover:text-foreground transition-colors cursor-pointer ${currentFolderId === null ? 'text-foreground font-bold' : 'text-slate-500'}`}
        >
          My Cloud Drive
        </button>
        ...
        {breadcrumbs.map(crumb => (
          <React.Fragment key={crumb.id}>
            ...
            <button
              ...
              className={`hover:text-foreground transition-colors cursor-pointer ${currentFolderId === crumb.id ? 'text-foreground font-bold' : 'text-slate-500'}`}
            >
              {crumb.name}
            </button>
          </React.Fragment>
        ...
        {currentFolderId && (
          <button
            ...
            className="ml-auto flex items-center gap-1 text-[10px] text-slate-500 hover:text-foreground font-bold bg-slate-950/40 border border-border px-2 py-0.5 rounded transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            ...
            {currentFiles.filter(f => f.isFolder).map(fld => (
              <div
                ...
                className="p-4 bg-card border border-border hover:border-violet-500/35 hover:bg-slate-900/10 rounded-xl cursor-pointer shadow-md transition-all flex items-center gap-3 text-left"
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 font-semibold">Folder</p>
                </div>
              </div>
            ))}
            ...
            {currentFiles.filter(f => !f.isFolder).map(file => {
              ...
              return (
                <div
                  ...
                  className="p-4 bg-card border border-border hover:border-violet-500/35 hover:bg-slate-900/10 rounded-xl cursor-pointer shadow-md transition-all flex items-center gap-3 text-left"
                >
                  <div className={`w-10 h-10 rounded-lg bg-slate-950 border border-border flex items-center justify-center font-bold shrink-0 ${colorStyle}`}>
                    <FileIcon className="w-5.5 h-5.5" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-foreground truncate">{file.name}</p>
                    <p className="text-[10px] text-slate-500">{file.size} • v{file.version}</p>
                  </div>
                </div>
              );
            })}
          ...
          {currentFiles.length === 0 && (
            <div className="text-center py-20 bg-slate-900/10 border border-dashed border-border rounded-xl text-slate-500 text-xs">
              📂 Empty Folder. Choose files to upload.
            </div>
          )}
        </div>

        {/* Right Upload Panel */}
        <div className="lg:col-span-4 bg-card border border-border rounded-2xl p-6 space-y-4">
          ...
          <form onSubmit={handleUploadFile} className="space-y-4 text-xs">
            <div>
              <label className="block text-slate-400 mb-1.5 font-medium">File Name (No extension)</label>
              <input
                type="text"
                required
                ...
                className="w-full bg-slate-900 border border-border rounded-lg p-2.5 text-white outline-none focus:border-violet-500"
              />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-slate-400 mb-1.5 font-medium">Extension</label>
                <select
                  value={uploadFileType}
                  onChange={(e) => setUploadFileType(e.target.value)}
                  className="w-full bg-slate-900 border border-border rounded-lg p-2.5 outline-none focus:border-violet-500 text-slate-350"
                >
              ...
              <div>
                <label className="block text-slate-400 mb-1.5 font-medium">Size</label>
                <input
                  type="text"
                  value={uploadFileSize}
                  onChange={(e) => setUploadFileSize(e.target.value)}
                  placeholder="2.4 MB"
                  className="w-full bg-slate-900 border border-border rounded-lg p-2.5 outline-none focus:border-violet-500 text-white"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="w-full bg-violet-600 hover:bg-violet-500 text-white font-bold py-2.5 rounded-xl transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer"
            >
      ...
      {folderModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-2xl p-6 w-full max-w-sm text-foreground">
            <h2 className="text-sm font-bold mb-4 pb-2 border-b border-border">Create Folder</h2>
            <form onSubmit={handleCreateFolder} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Folder Name</label>
                <input
                  type="text"
                  required
                  autoFocus
                  ...
                  className="w-full bg-slate-900 border border-border rounded-lg p-2 text-xs focus:border-violet-500 outline-none text-white"
                />
              </div>
              <div className="flex justify-end gap-2 pt-2">
                <button
                  ...
                  className="bg-slate-800 hover:bg-slate-700 text-slate-350 font-semibold px-4 py-2 rounded-lg text-xs cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="bg-violet-650 hover:bg-violet-600 text-white font-bold px-4 py-2 rounded-lg text-xs cursor-pointer"
                >
                  Create Folder
                </button>
              </div>
      ...
      {selectedFileId && selectedFile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col text-foreground">
            
            <div className="p-4 border-b border-border flex items-center justify-between">
              <div className="min-w-0 pr-4 flex items-center gap-2">
                <span className="text-xl">📄</span>
                <div>
                  <h2 className="text-xs font-bold truncate leading-tight">{selectedFile.name}</h2>
                  <p className="text-[9px] text-slate-500 leading-none">Size: {selectedFile.size} • v{selectedFile.version}</p>
                </div>
              </div>
              <button onClick={() => setSelectedFileId(null)} className="text-slate-400 hover:text-white p-1 cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              <div className="md:col-span-7 space-y-4">
                <div className="h-64 rounded-xl border border-border bg-slate-900/10 flex flex-col items-center justify-center text-center p-6 relative">
                  <span className="text-4xl mb-3 font-semibold">📂</span>
                  <p className="text-xs font-semibold text-slate-400 truncate max-w-sm">Preview of "{selectedFile.name}"</p>
                  <p className="text-[10px] text-slate-500 mt-1.5">Secure Object Storage Node</p>
                  
                  <a
                    href="#"
                    onClick={(e) => e.preventDefault()}
                    className="mt-4 bg-slate-800 hover:bg-slate-700 border border-border text-slate-200 font-bold px-4 py-2 rounded-lg text-xs transition-all flex items-center gap-1.5"
                  >
                    <ExternalLink className="w-3.5 h-3.5" /> Download File
                  </a>
                </div>

                <div className="p-3 bg-slate-950/30 border border-border rounded-xl text-xs space-y-1.5">
                  <div className="flex items-center gap-1 text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">
                    <Share2 className="w-3.5 h-3.5" /> Visibility Access
                  </div>
              ...
              <div className="md:col-span-5 space-y-5">
                <div>
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1">
                    <Clock className="w-4 h-4 text-violet-400" /> Version History
                  </h3>
                  <div className="p-3 bg-slate-950/40 border border-border rounded-xl space-y-3 max-h-[160px] overflow-y-auto pr-1 text-[10px]">
                    {selectedFile.history?.map(hist => (
                      <div key={hist.version} className="space-y-1 pb-1.5 border-b border-border last:border-0 last:pb-0">
                        <div className="flex justify-between font-bold text-slate-350">
                          <span>Version {hist.version}</span>
                          <span className="text-slate-500">{new Date(hist.updatedAt).toLocaleDateString()}</span>
                        </div>
                        <p className="text-slate-450 leading-relaxed">{hist.description}</p>
                        <p className="text-[9px] text-slate-500 font-semibold">Modified by: {hist.updatedBy}</p>
                      </div>
                    )) || (
                ...
                <div>
                  ...
                  <form onSubmit={handleShareFile} className="flex gap-1.5">
                    <select
                      ...
                      className="flex-1 bg-slate-900 border border-border rounded-lg p-2 text-xs text-slate-400 outline-none"
                    >
                    ...
                    <button
                      ...
                      className="bg-violet-650 hover:bg-violet-600 text-white font-bold text-[10px] px-3.5 rounded-lg disabled:opacity-40 cursor-pointer"
                    >
                      Share
                    </button>
                  </form>
                ...
                {(selectedFile.ownerId === user.id || user.role === 'Super Admin') && (
                  <button
                    ...
                    className="w-full mt-4 bg-red-950/20 hover:bg-red-950/40 border border-red-500/20 hover:border-red-500/40 text-red-400 font-bold py-2 rounded-xl text-xs flex items-center justify-center gap-1.5 active:scale-95 transition-all cursor-pointer"
                  >
