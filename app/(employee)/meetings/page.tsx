...
export default function VideoMeetings() {
  ...
  return (
    <div className="max-w-7xl mx-auto h-[calc(100vh-10rem)] overflow-hidden flex flex-col">
      
      {!activeCallRoomId ? (
        <div className="flex-1 overflow-y-auto space-y-6">
          <div className="p-6 rounded-2xl border border-border bg-card text-left">
            <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
              <Video className="w-5 h-5 text-rose-500" /> Unified Meet Center
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
            ...
            <div className="md:col-span-5 bg-card border border-border rounded-xl p-6 space-y-4">
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-400">Start a Meeting</h2>
              
              <div className="space-y-3.5 text-xs">
                <div>
                  <label className="block text-slate-400 mb-1.5 font-medium">Meeting Subject / Title</label>
                  <input
                    type="text"
                ...
                <button
                  ...
                  className="w-full bg-rose-600 hover:bg-rose-500 text-white font-bold py-2.5 rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
            ...
            <div className="md:col-span-7 space-y-6">
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                  <Radio className="w-4 h-4 text-emerald-500 animate-pulse" /> Active Discussions Now ({activeRoomsList.length})
                </h2>
                ...
                {activeRoomsList.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {activeRoomsList.map(room => {
                      ...
                      return (
                        <div key={room.id} className="p-3.5 rounded-xl border border-border bg-slate-900/10 dark:bg-slate-950/20 flex items-center justify-between gap-4">
                          ...
                          <button
                            ...
                            className="bg-rose-600 hover:bg-rose-500 text-white font-bold px-4 py-2 rounded-lg text-xs shrink-0 cursor-pointer"
                          >
                            Join Call
                          </button>
                        </div>
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Upcoming Scheduled Conferences</h2>
                ...
                {scheduledRoomsList.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {scheduledRoomsList.map(room => (
                      <div key={room.id} className="p-3 rounded-xl border border-border bg-slate-900/10 dark:bg-slate-950/20 flex items-center justify-between text-xs">
                        <div>
                          <p className="font-bold text-foreground">{room.title}</p>
                        ...
                        <button
                          onClick={() => handleJoinMeetingRoom(room)}
                          className="bg-slate-850 hover:bg-slate-750 text-slate-200 border border-border font-bold px-3 py-1.5 rounded text-[10px]"
                        >
                          Join Lobby
                        </button>
                      </div>
        ...
        // Active Jitsi view container
        <div className="flex-1 flex flex-col overflow-hidden border border-border bg-slate-950 rounded-2xl relative h-full min-h-[400px]">
          <div className="flex-1 w-full h-full min-h-[350px]" id="jitsi-container" />

          {/* Controls Bar */}
          <div className="absolute top-4 left-4 z-40 bg-card border border-border backdrop-blur-md px-4 py-2 rounded-xl flex items-center gap-3 shadow-2xl">
            <span className="text-xs font-bold text-foreground shrink-0">{activeRoom?.title}</span>
            
            <button
              onClick={() => {
                if (typeof window !== 'undefined' && activeRoom) {
                  const inviteUrl = `${window.location.origin}/meetings?room=${activeRoom.id}`;
                  navigator.clipboard.writeText(inviteUrl);
                  alert(`Invite Link copied to clipboard!\n${inviteUrl}\nShare this with other employees to let them join.`);
                }
              }}
              className="px-3 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-[10px] font-bold flex items-center gap-1.5 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-red-650 hover:bg-red-500 text-white text-[10px] font-bold transition-all cursor-pointer"
            >
              Leave Call
            </button>
          </div>
